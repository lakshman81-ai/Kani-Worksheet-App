      ...
      {currentPage === 'spellChallenge' && (
        <div style={styles.spellContainer}>
          ...
          <div style={styles.gameCard}>
            <div style={styles.categoryChip}><span>📂</span><span>{(wordList[currentIndex] as SpellWord)?.category}</span></div>
            <div style={{ ...styles.difficultyChip, background: (wordList[currentIndex] as SpellWord)?.difficulty === 'easy' ? '#4caf50' : (wordList[currentIndex] as SpellWord)?.difficulty === 'medium' ? '#ff9800' : '#f44336' }}>{(wordList[currentIndex] as SpellWord)?.difficulty === 'easy' ? '🌟 Easy' : (wordList[currentIndex] as SpellWord)?.difficulty === 'medium' ? '⭐ Medium' : '💪 Hard'}</div>
            <div style={styles.speakerSection}>
              <button style={{ ...styles.speakerBtn, ...(isSpeaking ? styles.speakerActive : {}) }} onClick={() => speakWord((wordList[currentIndex] as SpellWord)?.word)} disabled={isSpeaking}><span style={styles.speakerEmoji}>{isSpeaking ? '🔊' : '🔈'}</span></button>
              <p style={styles.speakerHint}>{isSpeaking ? '🎵 Speaking...' : '👆 Tap to hear the word'}</p>
            </div>
            {spellMode === 'fillIn' && <div style={styles.fillInBox}><span style={styles.fillInPattern}>{(wordList[currentIndex] as SpellWord)?.fillInBlank}</span><p style={styles.fillInHint}>Fill in the missing letters</p></div>}
            ...
            {showHint && !feedback && <div style={styles.hintBox}><span style={styles.hintIcon}>💡</span><p style={styles.hintText}>{(wordList[currentIndex] as SpellWord)?.hint}</p></div>}
          ...
          <div style={styles.bottomBar}>
            <div style={styles.leftActions}><button style={styles.repeatBtn} onClick={() => speakWord((wordList[currentIndex] as SpellWord)?.word)} disabled={isSpeaking}><span>🔄</span><span>Repeat</span></button></div>
      ...
      {currentPage === 'meaning' && (
        <div style={styles.meaningContainer}>
          ...
          <div style={styles.meaningCard}>
            <div style={{ ...styles.meaningDiffBadge, background: (wordList[currentIndex] as MeaningWord)?.difficulty === 'easy' ? '#4caf50' : (wordList[currentIndex] as MeaningWord)?.difficulty === 'medium' ? '#ff9800' : '#f44336' }}>{(wordList[currentIndex] as MeaningWord)?.difficulty === 'easy' ? '🌟 Easy' : (wordList[currentIndex] as MeaningWord)?.difficulty === 'medium' ? '⭐ Medium' : '💪 Hard'}</div>
            <div style={styles.wordSection}><p style={styles.wordPrompt}>What does this word mean?</p><div style={styles.wordBox}><span style={styles.bigWord}>{(wordList[currentIndex] as MeaningWord)?.word}</span></div></div>
            <div style={styles.exampleSection}><span style={styles.exampleIcon}>💬</span><p style={styles.exampleText}><strong>Example:</strong> {(wordList[currentIndex] as MeaningWord)?.example}</p></div>
            ...
            {showHint && !feedback && <div style={styles.synonymBox}><span style={styles.synonymLabel}>💡 Similar words:</span><div style={styles.synonymTags}>{(wordList[currentIndex] as MeaningWord)?.synonyms.map((syn, i) => (<span key={i} style={styles.synonymTag}>{syn}</span>))}</div></div>}
...
// ... (keyframes and styles remain the same)
